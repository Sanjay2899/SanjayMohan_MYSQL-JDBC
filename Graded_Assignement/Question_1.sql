...
--
/*!50003 DROP PROCEDURE IF EXISTS `SupplierServiceRating` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SupplierServiceRating`()
BEGIN
SELECT S.SUPP_ID, S.SUPP_NAME, 
           ...
           from supplier S join supplier_pricing SP on
           S.SUPP_ID=SP.SUPP_ID join orders O on O.PRICING_ID=SP.PRICING_ID join Rating R on R.ORD_ID=O.ORD_ID group by S.SUPP_ID, S.SUPP_NAME;
END ;;
DELIMITER ;
...
/*!50001 VIEW `lowest_expensive_product` AS select `c`.`CAT_ID` AS `CAT_ID`,`c`.`CAT_NAME` AS `CAT_NAME`,`p`.`PRO_NAME` AS `PRO_NAME`,`sp`.`SUPP_PRICE` AS `LowestPrice` from ((`product` `p` join `supplier_pricing` `sp` on((`sp`.`PRO_ID` = `p`.`PRO_ID`))) join `category` `c` on((`c`.`CAT_ID` = `p`.`CAT_ID`))) where (`c`.`CAT_ID`,`sp`.`SUPP_PRICE`) in (select `c`.`CAT_ID`,min(`sp`.`SUPP_PRICE`) AS `LowestPrice` from ((`product` `p` join `supplier_pricing` `sp` on((`sp`.`PRO_ID` = `p`.`PRO_ID`))) join `category` `c` on((`c`.`CAT_ID` = `p`.`CAT_ID`))) group by `c`.`CAT_ID`) */;
...
-- Dump completed on 2024-01-14 19:46:21
