...
/*!50001 VIEW `lowest_expensive_product` AS select `c`.`CAT_ID` AS `CAT_ID`,`c`.`CAT_NAME` AS `CAT_NAME`,`p`.`PRO_NAME` AS `PRO_NAME`,min(`sp`.`SUPP_PRICE`) AS `LowestPrice` from ((`category` `c` join `product` `p` on((`c`.`CAT_ID` = `p`.`CAT_ID`))) join `supplier_pricing` `sp` on((`sp`.`PRO_ID` = `p`.`PRO_ID`))) group by `c`.`CAT_ID`,`c`.`CAT_NAME`,`p`.`PRO_NAME` */;
...
-- Dump completed on 2024-01-11 22:02:13
